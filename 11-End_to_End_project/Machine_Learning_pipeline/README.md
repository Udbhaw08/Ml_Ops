...
These stages are already defined in `dvc.yaml`; the commands below are kept here for reference if you ever need to recreate them. Run `dvc repro` for normal use instead of re-adding stages.

Git Bash / Linux / macOS (`\` continuation):
...
Windows PowerShell (`` ` `` continuation — `\` is not a line continuation in PowerShell):
